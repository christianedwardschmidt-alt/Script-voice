  ...
  return (
    <div style={{ padding: '28px 28px', background: '#f8f7fc', minHeight: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 700, color: '#111827', letterSpacing: '-0.4px' }}>Clients</h1>
          <p style={{ color: '#9ca3af', fontSize: 14, marginTop: 2 }}>Manage your client relationships</p>
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="card" style={{ padding: 60, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
          ...
          <p style={{ color: '#9ca3af', fontSize: 15 }}>No clients found</p>
          <button className="btn-primary">
            ...
            <div key={client.id} className="card card-hover" style={{ padding: 20 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
                <div style={{ display: 'flex', gap: 12 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 600, color: '#111827' }}>{client.name}</div>
                    <div style={{ fontSize: 13, color: '#9ca3af' }}>{client.company}</div>
                  </div>
                </div>
              ...
              <div style={{ borderTop: '1px solid #f3f4f6', paddingTop: 12, display: 'flex', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontSize: 16, fontWeight: 700, color: '#111827' }}>${client.revenue.toLocaleString()}</div>
                  <div style={{ fontSize: 11, color: '#9ca3af' }}>Total revenue</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 16, fontWeight: 700, color: '#111827' }}>{client.projects}</div>
                  <div style={{ fontSize: 11, color: '#9ca3af' }}>Projects</div>
                </div>
