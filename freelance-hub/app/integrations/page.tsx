...
const integrations = [
  ...
  { id: 'notion', name: 'Notion', desc: 'All-in-one workspace for notes and docs', icon: '◼', connected: true, lastSync: '5 min ago', category: 'Productivity', color: '#374151' },
  ...
  { id: 'github', name: 'GitHub', desc: 'Version control and code collaboration', icon: '🐙', connected: true, lastSync: '3 min ago', category: 'Development', color: '#374151' },
  ...
  { id: 'vercel', name: 'Vercel', desc: 'Frontend deployment and edge network', icon: '▲', connected: false, lastSync: null, category: 'Development', color: '#374151' },
  ...
  return (
    <div style={{ padding: '28px 28px', background: '#f8f7fc', minHeight: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 700, color: '#111827', letterSpacing: '-0.4px' }}>Workspace</h1>
          <p style={{ color: '#9ca3af', fontSize: 14, marginTop: 2 }}>
          ...
          <div key={integ.id} className="card" style={{ padding: 18 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: '#f9fafb', border: '1px solid #f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>
                  {integ.icon}
                </div>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: '#111827' }}>{integ.name}</div>
                  {integ.connected && (
              ...
              <button
                ...
                style={{
                  ...
                  border: integ.connected ? '1px solid #e5e7eb' : 'none',
                }}
              >
                {integ.connected ? <><Check size={11} /> Connected</> : <><Plus size={11} /> Connect</>}
              </button>
            </div>
            <p style={{ fontSize: 12, color: '#9ca3af', marginBottom: integ.connected ? 10 : 0, lineHeight: 1.4 }}>{integ.desc}</p>
            {integ.connected && (
              <div style={{ display: 'flex', gap: 6 }}>
                <button style={{ flex: 1, padding: '6px 0', borderRadius: 7, background: '#f9fafb', border: '1px solid #f3f4f6', color: '#6b7280', fontSize: 11, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                  <RefreshCw size={10} /> Sync
                </button>
                <button style={{ flex: 1, padding: '6px 0', borderRadius: 7, background: '#f9fafb', border: '1px solid #f3f4f6', color: '#6b7280', fontSize: 11, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                  <ExternalLink size={10} /> Open
                </button>
                <button style={{ width: 30, height: 28, borderRadius: 7, background: '#f9fafb', border: '1px solid #f3f4f6', color: '#9ca3af', fontSize: 11, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Settings size={11} />
                </button>
              </div>
            )}
