...
const integrations = [
  ...
  { id: 'notion', name: 'Notion', desc: 'All-in-one workspace for notes and docs', icon: '◼', connected: true, lastSync: '5 min ago', category: 'Productivity', color: 'var(--text)' },
  ...
  { id: 'github', name: 'GitHub', desc: 'Version control and code collaboration', icon: '🐙', connected: true, lastSync: '3 min ago', category: 'Development', color: 'var(--text)' },
  ...
  { id: 'vercel', name: 'Vercel', desc: 'Frontend deployment and edge network', icon: '▲', connected: false, lastSync: null, category: 'Development', color: 'var(--text)' },
  ...
  return (
    <div style={{ padding: '28px 28px', background: 'var(--bg)', minHeight: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 700, color: 'var(--text)', letterSpacing: '-0.4px' }}>Workspace</h1>
          ...
          <div key={integ.id} className="card" style={{ padding: 18 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: 'var(--bg-2)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>
                  {integ.icon}
                </div>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)' }}>{integ.name}</div>
                  {integ.connected && (
              ...
              <button
                ...
                style={{
                  ...
                  border: integ.connected ? '1px solid var(--border)' : 'none',
                }}
              >
                {integ.connected ? <><Check size={11} /> Connected</> : <><Plus size={11} /> Connect</>}
              </button>
            </div>
            <p style={{ fontSize: 12, color: 'var(--text-2)', marginBottom: integ.connected ? 10 : 0, lineHeight: 1.4 }}>{integ.desc}</p>
            {integ.connected && (
              <div style={{ display: 'flex', gap: 6 }}>
                <button style={{ flex: 1, padding: '6px 0', borderRadius: 7, background: 'var(--bg-2)', border: '1px solid var(--border)', color: 'var(--text-2)', fontSize: 11, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                  <RefreshCw size={10} /> Sync
                </button>
                <button style={{ flex: 1, padding: '6px 0', borderRadius: 7, background: 'var(--bg-2)', border: '1px solid var(--border)', color: 'var(--text-2)', fontSize: 11, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }}>
                  <ExternalLink size={10} /> Open
                </button>
                <button style={{ width: 30, height: 28, borderRadius: 7, background: 'var(--bg-2)', border: '1px solid var(--border)', color: 'var(--text-2)', fontSize: 11, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Settings size={11} />
                </button>
              </div>
            )}
