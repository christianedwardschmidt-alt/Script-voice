  ...
  return (
    <div style={{ padding: '28px 28px', background: 'var(--bg)', minHeight: '100%' }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 26, fontWeight: 700, color: '#1a1535', letterSpacing: '-0.4px' }}>Community</h1>
        <p style={{ color: '#6b6899', fontSize: 14, marginTop: 2 }}>Connect, share, and grow with fellow freelancers</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr 260px', gap: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div className="card" style={{ padding: 18 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14, fontWeight: 600, fontSize: 14, color: '#1a1535' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {trending.map((t, i) => (
                <div key={t.tag} style={{ cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <div style={{ fontSize: 11, color: '#6b6899' }}>#{i + 1} freelance</div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#7c3aed' }}>{t.tag}</div>
                    <div style={{ fontSize: 11, color: '#6b6899' }}>{t.posts}</div>
                  </div>
              ...
              {suggested.map(s => (
                <div key={s.name} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#1a1535', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.name}</div>
                    <div style={{ fontSize: 11, color: '#7c3aed' }}>{s.handle}</div>
                    <div style={{ fontSize: 11, color: '#6b6899' }}>{s.followers} followers</div>
                  </div>
        ...
        <div>
          ...
          {/* Tabs */}
          <div style={{ display: 'flex', gap: 0, borderBottom: '1px solid rgba(120,100,200,0.1)', marginBottom: 14 }}>
            {tabs.map(tab => (
              <button
                key={tab}
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
              ...
              <div style={{ flex: 1 }}>
                <textarea
                  ...
                  style={{ width: '100%', background: 'none', border: 'none', fontSize: 15, color: '#1a1535', resize: 'none', outline: 'none', minHeight: 56, fontFamily: 'inherit', lineHeight: 1.5 }}
                />
                <div style={{ borderTop: '1px solid rgba(120,100,200,0.1)', paddingTop: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {displayed.map(post => (
              <div key={post.id} className="card" style={{ padding: 0, overflow: 'hidden' }}>
                ...
                <div style={{ padding: 18 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                      ...
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          <span style={{ fontSize: 15, fontWeight: 700, color: '#1a1535' }}>{post.author}</span>
                        ...
                        <div style={{ fontSize: 12, color: '#6b6899' }}>{post.handle} · {post.role} · {post.time} ago</div>
                      </div>
                    </div>
                  ...
                  {/* Content */}
                  <p style={{ fontSize: 14, color: '#1a1535', lineHeight: 1.7, whiteSpace: 'pre-line', marginBottom: post.image ? 14 : 0 }}>
                    {post.content}
                  </p>

                  {/* Image */}
                  {post.image && (
                    <div style={{ borderRadius: 12, overflow: 'hidden', border: '1px solid rgba(120,100,200,0.1)', marginBottom: 0 }}>
                      <div style={{ height: 220, background: post.image.grad, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, position: 'relative' }}>
                        ...
                        <div style={{ fontSize: 14, fontWeight: 600, color: '#1a1535', background: 'rgba(255,255,255,0.85)', padding: '4px 14px', borderRadius: 20, backdropFilter: 'blur(4px)' }}>
                          {post.image.label}
                        </div>
                  ...
                  <div style={{ display: 'flex', gap: 14, marginTop: 12, marginBottom: 10 }}>
                    <span style={{ fontSize: 12, color: '#6b6899', display: 'flex', alignItems: 'center', gap: 3 }}>
                      <Eye size={12} /> {fmtNum(post.views)} views
                    </span>
                    <span style={{ fontSize: 12, color: '#6b6899' }}>{post.comments} comments</span>
                    <span style={{ fontSize: 12, color: '#6b6899' }}>{post.shares} reposts</span>
                  </div>

                  {/* Action bar */}
                  <div style={{ borderTop: '1px solid rgba(120,100,200,0.1)', paddingTop: 10, display: 'flex', gap: 4, justifyContent: 'space-between' }}>
                    <button
                    ...
                    <button style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'none', border: 'none', cursor: 'pointer', color: '#6b6899', fontSize: 13, padding: '7px 14px', borderRadius: 8 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {/* Upcoming Events */}
          <div className="card" style={{ padding: 18 }}>
            <div style={{ fontWeight: 600, fontSize: 14, color: '#1a1535', marginBottom: 14 }}>Upcoming Events</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[
                ...
                <div key={i} style={{ paddingBottom: i < 2 ? 12 : 0, borderBottom: i < 2 ? '1px solid rgba(120,100,200,0.1)' : 'none' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#1a1535', marginBottom: 3 }}>{event.name}</div>
                  ...
                  <div style={{ fontSize: 12, color: '#6b6899' }}>{event.date}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Hashtag suggest */}
          <div className="card" style={{ padding: 18 }}>
            <div style={{ fontWeight: 600, fontSize: 14, color: '#1a1535', marginBottom: 12 }}>Your Top Tags</div>
