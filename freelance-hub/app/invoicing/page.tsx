  ...
  return (
    <div style={{ padding: '28px 28px', background: 'var(--bg)', minHeight: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 700, color: '#1a1535', letterSpacing: '-0.4px' }}>Billing</h1>
          <p style={{ color: '#6b6899', fontSize: 14, marginTop: 2 }}>Create, send and track invoices</p>
        </div>
        ...
        {[
          ...
          <div key={label} className="card card-hover" style={{ padding: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <div>
                <div style={{ fontSize: 11, color: '#6b6899', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>{label}</div>
                <div style={{ fontSize: 26, fontWeight: 700, color: '#1a1535' }}>{value}</div>
              </div>
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid rgba(120,100,200,0.1)' }}>
              {['Invoice', 'Client', 'Project', 'Amount', 'Status', 'Issued', 'Due', ''].map(h => (
                <th key={h} style={{ textAlign: 'left', fontSize: 11, color: '#6b6899', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', padding: '13px 18px' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
              ...
              <tr key={inv.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid #f9fafb' : 'none' }}>
                ...
                <td style={{ padding: '14px 18px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 500, color: '#1a1535' }}>{inv.client}</span>
                  </div>
                </td>
                <td style={{ padding: '14px 18px', fontSize: 13, color: '#6b6899' }}>{inv.project}</td>
                ...
                <td style={{ padding: '14px 18px', fontSize: 13, color: '#6b6899' }}>{inv.issued}</td>
                ...
                <td style={{ padding: '14px 18px' }}>
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button style={{ width: 28, height: 28, borderRadius: 7, background: 'var(--bg-2)', border: '1px solid rgba(120,100,200,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#6b6899' }}><Eye size={13} /></button>
                    {inv.status !== 'Paid' && (
                    ...
                    <button style={{ width: 28, height: 28, borderRadius: 7, background: 'var(--bg-2)', border: '1px solid rgba(120,100,200,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#6b6899' }}><Download size={13} /></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {showNew && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'var(--card)', border: '1px solid rgba(120,100,200,0.1)', borderRadius: 16, width: '90%', maxWidth: 680, maxHeight: '90vh', overflow: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '18px 24px', borderBottom: '1px solid rgba(120,100,200,0.1)' }}>
              <div>
                <div style={{ fontSize: 16, fontWeight: 700, color: '#1a1535' }}>New Invoice</div>
                <div style={{ fontSize: 12, color: '#6b6899' }}>INV-2026-092</div>
              </div>
              <button onClick={() => setShowNew(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#6b6899' }}><X size={18} /></button>
            </div>
            <div style={{ padding: 24 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
                {['Client Name', 'Client Email', 'Project Name', 'Due Date'].map(label => (
                  <div key={label}>
                    <label style={{ fontSize: 13, fontWeight: 500, color: '#1a1535', display: 'block', marginBottom: 5 }}>{label}</label>
                    <input className="search-input" style={{ paddingLeft: 12 }} placeholder={`Enter ${label.toLowerCase()}...`} />
                  </div>
                ))}
              </div>
              {/* Line Items */}
              <div style={{ fontWeight: 600, fontSize: 14, color: '#1a1535', marginBottom: 12 }}>Line Items</div>
              {lineItems.map((item, i) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '3fr 1fr 1fr 1fr', gap: 8, marginBottom: 8 }}>
                  {[item.description, item.qty, `$${item.rate}`, `$${item.total}`].map((val, j) => (
                    <div key={j} style={{ padding: '9px 12px', background: 'var(--bg-2)', border: '1px solid rgba(120,100,200,0.1)', borderRadius: 8, fontSize: 13, color: j === 3 ? '#10b981' : '#374151', fontWeight: j === 3 ? 600 : 400 }}>
                      {val}
                    </div>
                  ))}
                </div>
              ))}
              <div style={{ borderTop: '1px solid rgba(120,100,200,0.1)', paddingTop: 14, marginTop: 12, display: 'flex', justifyContent: 'flex-end' }}>
                <div style={{ width: 240 }}>
