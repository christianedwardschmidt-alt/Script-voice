...
const courses = [
  ...
  { id: 6, title: 'Content Marketing', instructor: 'Tom Blake', category: 'Marketing', duration: '7h 50m', lessons: 33, rating: 4.6, progress: 0, enrolled: false, price: 39, color: '#6b7280', badge: null },
]
  ...
  return (
    <div style={{ padding: '28px 28px', background: '#f8f7fc', minHeight: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 700, color: '#111827', letterSpacing: '-0.4px' }}>Upskilling</h1>
          <p style={{ color: '#9ca3af', fontSize: 14, marginTop: 2 }}>Level up your skills with curated courses</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 }}>
        {filtered.map(course => (
          <div key={course.id} className="card card-hover" style={{ overflow: 'hidden', padding: 0 }}>
            {/* Thumbnail */}
            <div style={{ height: 110, background: `linear-gradient(135deg, ${course.color}22, ${course.color}08)`, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', borderBottom: '1px solid #f3f4f6' }}>
              ...
              {course.badge && (
                <span style={{ position: 'absolute', top: 10, right: 10, fontSize: 10, fontWeight: 700, color: course.color, background: '#fff', padding: '2px 8px', borderRadius: 10, border: `1px solid ${course.color}30` }}>
                  {course.badge}
                </span>
              )}
            </div>
            <div style={{ padding: 16 }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 600, color: '#111827', marginBottom: 3, lineHeight: 1.4 }}>{course.title}</div>
              <div style={{ fontSize: 12, color: '#9ca3af', marginBottom: 10 }}>by {course.instructor}</div>
              ...
              {course.enrolled ? (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 16, fontWeight: 700, color: '#111827' }}>{course.price === 0 ? 'Free' : `$${course.price}`}</span>
