...
const courses = [
  ...
  { id: 6, title: 'Content Marketing', instructor: 'Tom Blake', category: 'Marketing', duration: '7h 50m', lessons: 33, rating: 4.6, progress: 0, enrolled: false, price: 39, color: 'var(--text-2)', badge: null },
]
  ...
  return (
    <div style={{ padding: '28px 28px', background: 'var(--bg)', minHeight: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 26, fontWeight: 700, color: 'var(--text)', letterSpacing: '-0.4px' }}>Upskilling</h1>
          <p style={{ color: 'var(--text-2)', fontSize: 14, marginTop: 2 }}>Level up your skills with curated courses</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 }}>
        {filtered.map(course => (
          <div key={course.id} className="card card-hover" style={{ overflow: 'hidden', padding: 0 }}>
            {/* Thumbnail */}
            <div style={{ height: 110, background: `linear-gradient(135deg, ${course.color}22, ${course.color}08)`, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', borderBottom: '1px solid var(--border)' }}>
            ...
            <div style={{ padding: 16 }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)', marginBottom: 3, lineHeight: 1.4 }}>{course.title}</div>
              <div style={{ fontSize: 12, color: 'var(--text-2)', marginBottom: 10 }}>by {course.instructor}</div>
              ...
              {course.enrolled ? (
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                    <span style={{ fontSize: 11, color: 'var(--text-2)' }}>Progress</span>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--text)' }}>{course.price === 0 ? 'Free' : `$${course.price}`}</span>
