...
const client = createClient({
  url: process.env.TURSO_DATABASE_URL ?? 'file:/tmp/centerflo.db',
...
declare global {
  ...
  var __centerfloInit: Promise<void> | undefined
}

function ensureReady(): Promise<void> {
  if (!globalThis.__centerfloInit) globalThis.__centerfloInit = runInit()
  return globalThis.__centerfloInit
}
...
async function seedPosts() {
  ...
  await client.batch([
    ...
    { sql, args: ['Tom Blake','@tomblake_brand','Brand Strategist','👨🏼‍💼','#06b6d4','2d',0,'My home office setup after 3 years of freelancing. The monitor arm was a game changer. 🖥\n\nTools I swear by:\n• Standing desk (health investment)\n• Good mic (clients notice)\n• Notion + CenterFlo for project tracking',JSON.stringify({type:'photo',label:'Home Office Setup',emoji:'🖥',grad:'linear-gradient(135deg, #cffafe 0%, #67e8f9 50%, #22d3ee 100%)'}),94,41,7,5200,0,0,1,new Date(now - 48*3600000).toISOString()] },
  ], 'write')
}
...
async function seedActivityLog() {
  await client.execute({
    sql: `INSERT INTO activity_log (message,createdAt) VALUES (?,?)`,
    args: ['Welcome to CenterFlo — your workspace is ready', new Date().toISOString()],
  })
}
