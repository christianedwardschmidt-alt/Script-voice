...
export async function POST(req: NextRequest) {
  ...
  const { name, icon, description, status, trigger_type, trigger_config, conditions, actions, template_id } = body
  const now = new Date().toISOString()
  const result = await execute(
    `INSERT INTO agents (user_id,name,icon,description,status,trigger_type,trigger_config,conditions,actions,template_id,run_count,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,0,?,?)`,
    [user.id, name ?? '', icon ?? '🤖', description ?? '', status ?? 'active', trigger_type ?? '', JSON.stringify(trigger_config ?? {}), JSON.stringify(conditions ?? []), JSON.stringify(actions ?? []), template_id ?? '', now, now]
  )
