...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const existing = await queryOne(`SELECT * FROM crm_clients WHERE id = ?`, [id])
  ...
  await execute(
    ...
    [next.name, next.company, next.email, next.phone, next.website, next.stage, next.value, next.avatar, next.avatarBg, next.tags, next.lastContact, next.starred, next.rating, next.notes, id]
  )
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const existing = await queryOne<{ name: string }>(`SELECT name FROM crm_clients WHERE id = ?`, [id])
  if (!existing) return NextResponse.json({ error: 'Not found' }, { status: 404 })
