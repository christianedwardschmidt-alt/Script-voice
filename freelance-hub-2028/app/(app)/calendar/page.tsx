...
export default function CalendarPage() {
  ...
  // weekOffset: 0 = week containing selected date, -1 = previous week, etc.
  ...
  // Fetch events for the current month view (desktop) or ±1 months (mobile)
  ...
  // Mobile week strip: week containing selected date + weekOffset
  const weekDays = useMemo(() => {
    const [y, m, d] = selected.split('-').map(Number)
    const sel = new Date(y, m - 1, d)
    const dow = sel.getDay()
    return Array.from({ length: 7 }, (_, i) => {
      const dt = new Date(y, m - 1, d - dow + i + weekOffset * 7)
      return dt
    })
  }, [selected, weekOffset])

  // When navigating weeks on mobile, keep selected within view
  function prevWeek() {
    setWeekOffset(o => o - 1)
    const first = weekDays[0]
    const prev = new Date(first.getFullYear(), first.getMonth(), first.getDate() - 7)
  ...
  function nextWeek() {
    ...
    const next = new Date(last.getFullYear(), last.getMonth(), last.getDate() + 7)
    ...
    return (
      <div className="page-pad" style={{ padding: '22px 16px 100px', background: 'var(--bg)', minHeight: '100dvh' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
          <button onClick={prevWeek} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8, padding: '6px 8px', cursor: 'pointer', display: 'flex', color: 'var(--text-2)' }}>
            <ChevronLeft size={15} />
          </button>
          <span style={{ flex: 1, fontSize: 13.5, fontWeight: 700, color: 'var(--text)', textAlign: 'center' }}>{weekLabel}</span>
          <button onClick={() => { setSelected(todayStr); setWeekOffset(0); setView(new Date(today.getFullYear(), today.getMonth(), 1)) }}
          ...
          <button onClick={nextWeek} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 8, padding: '6px 8px', cursor: 'pointer', display: 'flex', color: 'var(--text-2)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4, marginBottom: 22 }}>
          {weekDays.map(dt => {
            ...
            return (
              <button
                key={dateStr}
                onClick={() => { setSelected(dateStr); setView(new Date(dt.getFullYear(), dt.getMonth(), 1)) }}
                style={{
              ...
              >
                <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.5px', color: isSel ? 'rgba(255,255,255,0.8)' : 'var(--text-3)', textTransform: 'uppercase' }}>
                  {DAYS[dt.getDay()].slice(0, 2)}
                </span>
                <span style={{ fontSize: 14, fontWeight: 700, color: isSel ? '#fff' : isToday ? 'var(--green)' : 'var(--text)' }}>
                  {dt.getDate()}
                </span>
                <span style={{ width: 5, height: 5, borderRadius: '50%', background: hasDot ? (isSel ? 'rgba(255,255,255,0.7)' : '#00b857') : 'transparent' }} />
              </button>
            )
          })}
        </div>
        ...
        {selectedEvents.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            ...
            <div style={{ fontSize: 13, color: 'var(--text-3)', lineHeight: 1.6 }}>No events today.<br />Tap <b>New Event</b> to add one.</div>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              return (
                <div key={ev.id} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderLeft: `3px solid ${ev.color}`, borderRadius: 12, padding: '13px 15px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 14, fontWeight: 650, color: 'var(--text)', lineHeight: 1.3, marginBottom: 6 }}>{ev.title}</div>
                      ...
                      {ev.description && (
                        <div style={{ fontSize: 12, color: 'var(--text-3)', lineHeight: 1.5, marginTop: 6 }}>{ev.description}</div>
                      )}
                    ...
                    <button onClick={() => deleteEvent(ev.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', display: 'flex', padding: 4, flexShrink: 0 }}>
                      <Trash2 size={14} />
                    </button>
                  </div>
                </div>
        ...
        {/* Add Event Modal */}
        {showForm && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 100 }} onClick={() => setShowForm(false)}>
