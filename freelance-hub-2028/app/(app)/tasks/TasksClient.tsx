...
const IntegrationIcon = ({ name }: { name: string }) => {
  const icons: Record<string, string> = { figma: '🎨', slack: '💬', google: '🔵', github: '⚫' }
  return (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{
                    ...
                    color: task.checked ? '#9ca3af' : '#111827',
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                    <span className={priorityClass[task.priority]}><Flag size={9} /> {task.priority}</span>
                ...
                <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                  {task.integrations.map(integ => <IntegrationIcon key={integ} name={integ} />)}
                  <div style={{ position: 'relative' }}>
                    ...
                    {openMenuId === task.id && (
                      <div style={{
                        ...
                        boxShadow: '0 8px 24px rgba(0,0,0,0.12)', zIndex: 10, minWidth: 130,
                      }}>
                        <button
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }} onClick={() => setShowModal(false)}>
          <div className="card modal-card" style={{ width: 440, padding: 24 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <input className="search-input" placeholder="Task title" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} />
              <textarea className="search-input" placeholder="Description" rows={3} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} style={{ resize: 'vertical', fontFamily: 'inherit' }} />
