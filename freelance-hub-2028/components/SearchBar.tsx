...
function formatAnswer(text: string) {
  return text.split('\n').map((line, i) => {
    ...
    if (line.startsWith('• '))
      return <div key={i} style={{ paddingLeft: 12, color: 'var(--text-2)', fontSize: 12, marginBottom: 1, position: 'relative' }}><span style={{ position: 'absolute', left: 2, color: '#00b857' }}>•</span>{line.slice(2)}</div>
